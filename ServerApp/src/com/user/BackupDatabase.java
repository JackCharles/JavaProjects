...
public class BackupDatabase extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if ((us.getRole() & 16384) == 0)
		{
			request.getRequestDispatcher("PermDen.html").forward(request, response);
			return;
		}

		if (JdbcHelper.BackupDatabase())
		{
			JdbcHelper.AddLog(Tools.getDateTime(), us.getUserId(), us.getUserName(), "数据库备份", "数据库备份成功！");
			Tools.PrintSuccessMsg(request, response, "MainPage.jsp", "_top");
		}
		else
			Tools.PrintErrorMsg(request, response, "备份数据库失败：" + JdbcHelper.ErrorMsg);
	}
